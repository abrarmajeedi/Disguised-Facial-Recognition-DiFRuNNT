...
#read predictions given by Key-point detection model and convert every image name to person's name
#for example, S2_D4_17 becomes S2
...
ratio = 1
#function which gives ratio of two distances
...
#function which gives angles between two lines
angles = []
...
#gives all the 28 angles and 66 ratios of an input 
...
#converts 20 facial key-points of each image to a shape of a list of 20 tuples with (x,y) coordinates 
#then finds all the angles and ratios and gives output in the form of a csv file
tempcsv = []
df = pd.read_csv("predictions.csv",header = None)
